                          ...
                          new HorizontalStackLayout {
                              Children = {
                                ...
                                .Margins(3, 0, 3, 0),
                                 new Label()
                                .Font(size: 11)
                                .Bind(Label.TextProperty, nameof(PaymentDto.Time), convert: (DateTime value) => value.ToString("dd-MMM-yyyy"))
                              }
                          }.Row(SectionLevel.Second)
                          .Column(SectionLevel.Second),
                         new Label { TextColor = Colors.LightGreen }
                            .Font(size: 18)
                            .Bind(Label.TextProperty, nameof(PaymentDto.Amount), convert: (decimal value) => "₦" + string.Format("{0:N0}", value))
                            .MinWidth(95)
                            .Row(SectionLevel.First)
...
public partial class PaymentViewModel : BaseViewModel {
    [ObservableProperty]
    private ObservableCollection<PaymentDto> _items;
    ...
    private async void Load() {
        Items = new ObservableCollection<PaymentDto>() {
            ...
            new PaymentDto("Spectranet 4G max", "Internet", 30000, DateTime.Now.AddDays(-3))
        };
    }
}
